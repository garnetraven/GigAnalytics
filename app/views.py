...
def reports(request):
    ...
    week_start_date_str = request.GET.get('week_start_date', today - timedelta(days=today.weekday()))
    ...
    earnings_mean = np.mean(df['earnings']) if not df.empty else 0
    expenses_sum = np.sum(df['expenses']) if not df.empty else 0
    mileage_mean = np.mean(df['mileage']) if not df.empty else 0
